...
- **REMOTE_CHEMIN** : *secret commun (Organisation)*  
- **ADRESSE_GLOBAL** : *var par dépôt*  
- **ADRESSE_LOCAL** : *var par dépôt*  
- **$ENV** : `dev|homol|prod` (input `remote_env`)

## Secrets communs (Organisation)
Tous les secrets `REMOTE*`, `SSH_*`, `FTPS_*` sont définis **au niveau Organisation** (option *Selected repositories* possible) :  
